#!/bin/sh
echo "[(C)2012-ID-Informatik] Configure module: tiny_mce..."
#
# configure le module 'tiny_mce'
# Usage: [service]
#
...
case "$1" in
  configure)
    ...
    echo "\tconfigure..."
    php "$wfw_dir/req/add_module.php" site_path="$www_dir" module="tiny_mce" > /dev/null 2>&1
    exit 0
    ;;
  remove)
    # configure les fichiers "default.xml"
    echo "\tunconfigure..."
    php "$wfw_dir/req/rem_module.php" site_path="$www_dir" module="tiny_mce" > /dev/null 2>&1
    exit 0
    ;;
  save)
