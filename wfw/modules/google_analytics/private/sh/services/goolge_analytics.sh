#!/bin/sh
echo "[(C)2012-ID-Informatik] Configure module: google_analytics..."
#
# configure le module 'google_analytics'
# Usage: [service]
#
...
case "$1" in
  configure)
    ...
    echo "\tconfigure..."
    php "$wfw_dir/req/add_module.php" site_path="$www_dir" module="google_analytics" > /dev/null 2>&1
    exit 0
    ;;
  remove)
    # configure les fichiers "default.xml"
    echo "\tunconfigure..."
    php "$wfw_dir/req/rem_module.php" site_path="$www_dir" module="google_analytics" > /dev/null 2>&1
    exit 0
    ;;
  save)
